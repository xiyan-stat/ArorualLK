...
LK.auroral <- function(data1, data2, nmlat=363, nmlt=363, time_pt, ratio, NC=NULL, nlevel=NULL, 
  ...
  #        ratio = the ratio between interpolation (empirical data) 
  ...
  if(ratio >= 1/5 | ratio < 1/10) {print("Ratio should be greater than 1/10 and less than 1/5")}
  ...
  if (type == "LK"){
    ...
    index1 <- seq(1, n1, by= ceiling(n1/(n2*ratio)))
    
    dat1 <- inter[index1,]
    dat1$source <- "inter_sys"
  }else if(type == "random"){
    index2 <- sample(1:n1, floor(n2*ratio))
  ...
  p <- comb %>%
      ...
      facet_grid(.~source) +
      theme_bw() +
      ggtitle(paste("Time point:",time_pt, ", Ratio:", ratio))
  }
